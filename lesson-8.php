<?php
if(empty($_GET['file'])){
header('Location:'.$_SERVER['PHP_SELF'].'?'.'file=logo.png');
}else{echo "";}
?>
<html>
<head>
...
<br><br><br>
<div id="pesan">
	<center><a href=><img src=<?php
$file = $_GET['file'];
$ban = array('onAbort',
'onBlur','onChange','onClick','onDblClick','onDragDrop','onError','onFocus','onKeyDown','onKeyPress','onKeyUp','onLoad','onMouseDown','onMouseMove','onMouseOut','onMouseOver','onMouseUp','onMove','onReset','onResize','onSelect','onSubmit','onUnload','onscroll', 'onbort');
foreach($ban as $banned){
while(preg_match( '/'.$banned.'/i', $file)){
$file = preg_replace('/'.$banned.'/i', '', $file);}}
 echo htmlspecialchars($file); ?> height=300 width=350 alt="#" /></a></center>
	<br>

<h2 class='g'><b class='h'>Name : CyberBullet
</b></h2>
